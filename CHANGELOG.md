...
- In `CollectFeeMode::Compounding` pools, the fee generated by the `DEAD_LIQUIDITY` share is no longer stranded in the vault. It is now accumulated into `Pool.protocol_b_fee` and claimable via `claim_protocol_fee2`. The stranded fee is backfilled into `Pool.protocol_b_fee` once, on each pool's first swap or protocol fee claim after this fix.
