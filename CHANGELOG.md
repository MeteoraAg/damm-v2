...
- New implementation of `validate_single_swap_instruction` to allow unlimited stack height. However, it make `validate_single_swap_instruction` more strict by disallowing the same swap transaction in the same `slot`.
