...
- In `CollectFeeMode::Compounding` pools, the claiming fee generated by the `DEAD_LIQUIDITY` share is no longer stranded in the vault. It is settled into `Pool.protocol_b_fee` on every `claim_protocol_fee2`, tracked by the new `Pool.dead_liquidity_fee_checkpoint` field. Each settlement also reclassifies the `DEAD_LIQUIDITY` fee amount from `PoolMetrics.total_lp_b_fee` to `PoolMetrics.total_protocol_b_fee`, so `total_lp_b_fee` can decrease on a claim.
