...
- In `CollectFeeMode::Compounding` pools, the fee generated by `DEAD_LIQUIDITY` share is no longer stranded in the vault. It is now accumulated into `Pool.protocol_b_fee` and claimable via `claim_protocol_fee2`.
