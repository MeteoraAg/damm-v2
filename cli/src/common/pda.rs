...
use cp_amm::constants::seeds::{
    CONFIG_PREFIX, POOL_AUTHORITY_PREFIX, POOL_PREFIX, REWARD_VAULT_PREFIX, TOKEN_BADGE_PREFIX, TOKEN_VAULT_PREFIX
};
use cp_amm::ID;
...
pub fn derive_reward_vault_pda(index: u8, pool: Pubkey) -> Pubkey {
    Pubkey::find_program_address(&[REWARD_VAULT_PREFIX.as_ref(), pool.as_ref(), index.to_le_bytes().as_ref()], &ID).0
}
