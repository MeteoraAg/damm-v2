...
pub struct WithdrawInegibleRewardParams {
    pub pool: Pubkey,
    pub reward_index: u8,
}

pub fn withdraw_inegible_reward<C: Deref<Target = impl Signer> + Clone>(
    params: WithdrawInegibleRewardParams,
    program: &Program<C>,
    transaction_config: RpcSendTransactionConfig,
    compute_unit_price: Option<Instruction>
) -> Result<()> {
    let WithdrawInegibleRewardParams { pool, reward_index } = params;
    ...
    println!("Withdraw inegible reward pool {pool} Signature: {signature:#?}");
