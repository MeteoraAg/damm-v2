...
pub fn handle_remove_liquidity(
    ...
    let transfer_fee_excluded_token_a_amount =
        calculate_transfer_fee_excluded_amount(&ctx.accounts.token_a_mint, token_a_amount)?.amount;

    let transfer_fee_excluded_token_b_amount =
        calculate_transfer_fee_excluded_amount(&ctx.accounts.token_b_mint, token_b_amount)?.amount;

    // Slippage check
    require!(
        transfer_fee_excluded_token_a_amount >= token_a_amount_threshold,
        PoolError::ExceededSlippage
    );
    require!(
        transfer_fee_excluded_token_b_amount >= token_b_amount_threshold,
        PoolError::ExceededSlippage
    );
